...
Ipv4StaticRouting::AddNetworkRouteTo (Ipv4Address network, 
                                      ...
                                      uint32_t metric)
{
  NS_LOG_UNCOND( network << " " << networkMask << " " << nextHop << " " << interface << " " << metric);
...
//change addNeighborRoute
void
Ipv4StaticRouting::AddNeighborRoute(Ipv4InterfaceAddress local,Ipv4InterfaceAddress neighbor )
{
	locallist.push_back(make_pair(local,neighbor));
...
Ipv4StaticRouting::LookupStatic (Ipv4Address dest, Ptr<NetDevice> oif)
{
 ...
 for(LocallistI l=locallist.begin();
	  l!=locallist.end();
	  l++
  )
 {
          Ipv4InterfaceAddress local=l->first;
          Ipv4InterfaceAddress neighbor=l->second;

         NS_LOG_UNCOND("Locallistlocal----"<<local);
          //NS_LOG_UNCOND("Locallist----"<<interface);
         NS_LOG_UNCOND("Locallistneighbor----"<<neighbor);
      ...
      if (mask.IsMatch (dest, entry)) 
        {
          ...
          NS_LOG_UNCOND("interfaceIdx"<<interfaceIdx);
          rtentry = Create<Ipv4Route> ();
          rtentry->SetDestination (route->GetDest ());
          rtentry->SetSource (SourceAddressSelection (interfaceIdx, route->GetDest ()));
          rtentry->SetGateway (route->GetGateway ());
          rtentry->SetOutputDevice (m_ipv4->GetNetDevice (interfaceIdx));
          rtentry->SetMetric(metric);
          LocallistI l=locallist.begin();
          Ipv4InterfaceAddress local=l->first;
          rtentry->SetNewSource(local.GetLocal());
          NS_LOG_UNCOND("Shortest_hopcount"<<rtentry->GetMetric());
          NS_LOG_UNCOND("Destination"<<rtentry->GetDestination());
          NS_LOG_UNCOND("Source"<<rtentry->GetSource());
          NS_LOG_UNCOND("OutputDevice"<<rtentry->GetOutputDevice());
        }
    }
  if (rtentry != 0)
    {
      NS_LOG_LOGIC ("Matching route via " << rtentry->GetGateway () << " at the end");
      NS_LOG_UNCOND("Matching route via " << rtentry->GetGateway () << " at the end");
    }
  else
    {
                                ...
                                LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  //find the neighbor    add by Tao
  uint16_t longest_mask = 0;
   Ipv4Address cursource=ipHeader.GetSource();
   for(LocallistI l=locallist.begin();
   	  l!=locallist.end();
   	  l++
     ) {
	               Ipv4InterfaceAddress neighbor=l->second;
	               Ipv4Mask mask = neighbor.GetMask();
	               uint16_t masklen = mask.GetPrefixLength ();
	               Ipv4Address entry = neighbor.GetLocal();

	      if (mask.IsMatch (cursource, entry))
	        {
	          ...
	          longest_mask = masklen;
	          rtentry->SetOldSource(entry);
	          NS_LOG_UNCOND("OldSource"<<rtentry->GetOldSource());
	        }
	    }
/*

      uint32_t a=cursource.Get()+rtentry->GetNewSource().Get()-rtentry->GetOldSource();
      Ipv4Address Aaddress=Ipv4Address(a);

      hrt.push_back(make_pair(Aaddress,cursource));
