...
Ipv4L3Protocol::IpForward (Ptr<Ipv4Route> rtentry, Ptr<const Packet> p, const Ipv4Header &header)
{
  ...
  //change
  uint32_t metric=rtentry->GetMetric();
  uint32_t curTtl=ipHeader.GetTtl();
   Ipv4Address  curSource=ipHeader.GetSource();
   NS_LOG_UNCOND("L3-Source" << curSource);

  if(metric<curTtl){
	 uint32_t numberAddress=curSource.Get()+rtentry->GetNewSource().Get()-rtentry->GetOldSource().Get();
	 uint32_t numberId=ipHeader.GetOriginalID().Get()+rtentry->GetNewSource().Get()-rtentry->GetOldSource().Get();
	 Ipv4Address newSource=Ipv4Address(numberAddress);
	 Ipv4Address newId=Ipv4Address(numberId);
	 ipHeader.SetTtl(metric);
	  ipHeader.SetSource(newSource);
	  ipHeader.SetOriginalId(newId);
  }
